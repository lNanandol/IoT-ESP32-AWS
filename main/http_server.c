...
/**
 * @brief Checks the g_fw_update_status and creates the fw_update _reset_timer if g_fw_update_status is true * 
 */
static void http_server_fw_update_reset_timer(void)
{
    if (g_fw_update_status == OTA_UPDATE_SUCESSFUL)
    {
        ESP_LOGI(TAG, "http_server_fw_update_reset_timer: FW update successful starting reset timer");

        // Give the web page the time to recevie and acknowledge back and initialize the timer
        ESP_ERROR_CHECK(esp_timer_create(&fw_update_reset_args, &fw_update_reset));
        ESP_ERROR_CHECK(esp_timer_start_once(fw_update_reset, 8000000)); 
    }
    else
    {
        ESP_LOGI(TAG, "http_server_fw_update_reset_timer: FW update unseccessful");
    }
}

/**
 * @brief HTTP server monitor task used to track event of the HTTP server
...
/**
 * @brief Sends the .ico (icon) when accessing the web page
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK  
 */
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    do
    {
        // Read the data form the request
        if ((recv_len = httpd_req_recv(req, ota_buff, MIN(content_lenght, sizeof(ota_buff)))) < 0)
        {
            ...
            if (recv_len == HTTPD_SOCK_ERR_TIMEOUT)
            {
                ...
                continue; ///> Retry recieven if timeout occurred
            }
            ESP_LOGI(TAG, "http_server_OTA_update_handler: OTA other Error %d", recv_len);
            return ESP_FAIL; 
        }
        printf("http_server_OTA_update_handler: OTA RX: %d of %d\r", content_recevie, content_lenght);

        // Is this the first data we are receiving
        // If so, it will have the information on the header that we need 
        if (!is_req_body_started)
        {
            ...
            if (err != ESP_OK)
            {
                printf("http_server_OTA_update_handler: Error with OTA beging, cancelling OTA\r\n");
                return ESP_FAIL;
            }
            else 
            {
            ...
            content_recevie += body_part_len;
        }
        else
        {
            // Write OTA data
            esp_ota_write(ota_handle, ota_buff, recv_len);
            content_recevie += recv_len;
        }
 
    }while (recv_len > 0 && content_recevie < content_lenght);
    ...
    // We wont update the global variables throughout the file, so lets send the message about the satus 
 ...
 * and respond with the compile timw/date when the page is first requested
...
/**
 * @brief DHT sensor readings JSON handler responds with DHT22 sensor data
 * @param req HTTP request for which the url needs to be handled 
 * @return ESP_OK 
 */
...
/**
 * @brief wifiConnect.json handler is invoce after the connect button is pressed 
...
static esp_err_t http_server_wifi_connect_status_json_handler(httpd_req_t *req)
{
    ...
    sprintf(statusJAON, "{\"wifi_connect_status\":%d}", g_wifi_connect_status);

    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, statusJAON, strlen(statusJAON));

    return ESP_OK;
}

/**
 * @brief wifiConnectInfo.json handler updates the connection information 
 * @param req HTTP request for wich the uri need to be handled 
 * @return ESP_OK
 */
...
/**
 * @brief wifiDisonnect.json handler responds by sneding a message to the Wifi application to disconnect
 * @param req HTTP request for wich the uri need to be handled 
 * @return ESP_OK
 */
...
static httpd_handle_t http_server_configure(void)
{
    ...
    // The core of the HTTP server will run on 
    config.core_id = HTTP_SERVER_TASK_CORE_ID;  

    // Adjust the default prorit to 1 less tha the wiFi application task 
    config.task_priority = HTTP_SERVER_TASK_PRIORITY;

    // Bump up the satck size (default is 4096)
