...
// Used to track the number of rerties when a connection attemp fails 
...
const int WIFI_APP_CONNECTING_USING_SAVED_CREDENTIALS_BIT	= BIT0;
...
/*
	...
	@param arg data, aside from event data, that is passed to the handler when is called 
	@param event_base the base id of the event to register the handler for 
	@param event_id the id from the event to register the hendler for 
	...
	if (event_base == WIFI_EVENT)
	{
		switch (event_id)
		{
			...
			case WIFI_EVENT_AP_STACONNECTED:
				ESP_LOGI(TAG, "WIFI_EVENT_AP_STA_CONNECTED");
				break;

			case WIFI_EVENT_AP_STADISCONNECTED:
				ESP_LOGI(TAG, "WIFI_EVENT_AP_STA_DISCONNECTED");
				break;
...
{
	// Event loop for the WiFi handler
...
/*
	Initializes the ICP stack and default WiFi configuration 
*/
static void wifi_app_default_wifi_init(void)
{
	// Initializes the ICP stack 
...
/*
	Configure the WiFi access point setting and assigns the static IP to the SoftIP
*/
static void wifi_app_soft_ap_config(void)
{
	// SoftIP - WiFi access point configuration 
	...
	inet_pton(AF_INET, WIFI_AP_IP, &ap_ip_info.ip);	//>> Assign access points static IP, Gateway and netmask
	...
	ESP_ERROR_CHECK(esp_netif_dhcps_start(esp_netif_ap));	//>> Start the AP DHCP server (for connecting status)
			...
			switch (msg.msgID)
			{
				case WIFI_APP_MSG_LOAD_SAVED_CREDENTIALS:
					...
					if(app_nvs_load_sta_creds())
					{
						...
						xEventGroupSetBits(wifi_app_event_group, WIFI_APP_CONNECTING_USING_SAVED_CREDENTIALS_BIT);
					}
					else
					{
				...
				case WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER:
					...
					// Let the HTTP server for the connection attempt
				...
				case WIFI_APP_MSG_STA_CONNECTED_GOT_IP:
					...
					if(eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDENTIALS_BIT)
					{
						xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_USING_SAVED_CREDENTIALS_BIT);
					}
					else 
					{
				...
				case WIFI_APP_MSG_STA_DISCONNECTED:
					...
					{
						ESP_LOGI(TAG, "WIFI_APP_MSG_STA_DISCONNECTED: Connection lost, retrying...");
						esp_wifi_connect();
						g_retry_number ++;
						break;
					} 
...
void wifi_app_start(void)
{
	...
	// Start WiFi start LED
	rgb_led_wifi_app_started();
	
	// Disable default WiFi logging messages
	esp_log_level_set("WiFi", ESP_LOG_NONE);

	// Allocate memory or the WiFi configuration
